document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Preloaded Sound Effects for Buttons ---
    const playBookConsultSound = () => {
        try {
            const audio = new Audio('sounds/BOOK.wav');
            audio.currentTime = 0;
            audio.play().catch(err => console.log('Book audio play blocked/failed:', err));
        } catch (e) {
    ...
    const playHannuAiSound = () => {
        try {
            ...
            audio.currentTime = 0;
            audio.play().catch(err => console.log('Hannu AI audio play blocked/failed:', err));
        } catch (e) {
    ...
    const playViewWorkSound = () => {
        try {
            ...
            audio.currentTime = 0;
            audio.play().catch(err => console.log('View work audio play blocked/failed:', err));
        } catch (e) {
            console.log('Error playing view work audio:', e);
        }
    };

    // Global Click Sound Delegation for ALL Buttons and Interactive Elements across the Website
    document.body.addEventListener('click', (e) => {
        ...
        const interactiveEl = e.target.closest('button, .button, .btn-cv, .nav-item, .btn-card-explore, .nav-sparkle-btn, .uiverse-ai-btn, .consultationModalBtn, .view-work-btn, .callModalBtn, .close-modal, .hamburger-menu, .sound-toggle-btn, .project-3d-card, .blog-3d-card, [role="button"]');
        if (!interactiveEl) return;

        const text = interactiveEl.textContent.toLowerCase();
        const href = interactiveEl.getAttribute('href') || '';

        // Determine which sound effect matches the button type
        if (interactiveEl.matches('#hannuAiBtn, .uiverse-ai-btn') || href.includes('hannu-ai.html') || text.includes("hannu's ai")) {
            playHannuAiSound();
        } else if (interactiveEl.matches('.consultationModalBtn, .book-call-btn') || text.includes('book') || text.includes('consult') || text.includes('inquire')) {
