document.addEventListener('DOMContentLoaded', () => {
    ...
    // Main Navigation Visibility Logic
    const navigation = document.getElementById("main-navigation");
    const hero = document.getElementById("home"); // The hero section has id="home"
    const about = document.getElementById("about");

    if (navigation && hero && about) {
        const observer = new IntersectionObserver(
            ([entry]) => {
                if (entry.isIntersecting) {
                    // Hero is visible → hide entire navigation
                    navigation.classList.remove("nav-visible");
                } else {
                    // Hero is no longer visible → show entire navigation
            ...
            const videoVisibilityObserver = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        // Only pause if user has actually scrolled away from the top hero section
  ...
  let heroHeight = window.innerHeight; // Approximate hero height, or could use document.querySelector('.hero').offsetHeight
  ...
  window.addEventListener('scroll', () => {
    ...
    // 1. Navigation visibility logic (hidden in hero, visible after)
    ...
    // 2. Hide on scroll down, show on scroll up (only relevant when visible)
  ...
  // --- Services Split-Layout Interactive Showcase Logic ---
  const initServicesShowcase = () => {
    const swiperEl = document.querySelector('.services-swiper');
    if (!swiperEl || typeof Swiper === 'undefined') return;

    // Services Data Array - Senior SaaS Storytelling Copy & Capability Checklists (No Emojis)
    const servicesData = [
      {
        cat: 'GROWTH MARKETING',
        title: 'Digital Marketing',
        desc: 'Marketing campaigns only succeed when messaging aligns directly with customer needs. We construct data-driven growth strategies that optimize every touchpoint from first impression to final conversion. By continuously testing and refining customer acquisition funnels, we help you lower customer acquisition costs and scale revenue.',
        bullets: [
          'Conversion Funnel Optimization',
          'HubSpot Automation',
          'Customer Acquisition Strategy',
          'Data-Driven ROI Analytics'
        ],
        link: 'https://blog.hubspot.com/',
        glow: 'rgba(168, 85, 247, 0.12)' // Purple
      },
      {
        cat: 'RESPONSIVE WEB DESIGN',
        title: 'Website Design',
        desc: 'Every website we build starts with a clear business objective. Before writing a single line of code, we understand how visitors move through your pages, where they lose interest, and what encourages them to take action. The result is a fast, responsive website designed to generate measurable business outcomes rather than simply looking attractive.',
        bullets: [
        ...
        link: 'https://web.dev/',
        glow: 'rgba(236, 72, 153, 0.12)' // Pink
      },
      {
        cat: 'ORGANIC ACQUISITION',
        title: 'SEO Optimization',
        desc: 'Sustainable search growth requires more than stuffing keywords into blog posts. We analyze search intent, resolve technical website bottlenecks, and build content structures that earn search engine authority. Our focus is on bringing qualified prospects to your site who are actively seeking the solutions you provide.',
        bullets: [
          'Technical SEO Audits',
          'Keyword & Intent Mapping',
          'Moz & Semrush Strategy',
          'Organic Lead Acquisition'
        ],
        link: 'https://moz.com/beginners-guide-to-seo',
        glow: 'rgba(59, 130, 246, 0.12)' // Blue
      },
      {
        cat: 'BUSINESS AUTOMATION',
        title: 'Process Automation',
        desc: 'Manual tasks slow teams down and increase the chance of mistakes. We design automation systems that connect your existing tools, remove repetitive work, and allow your team to focus on higher-value tasks. Every workflow is planned around your business processes, making automation reliable, scalable, and easy to maintain as your company grows.',
        bullets: [
          'Workflow Design',
          'API Integrations',
          'CRM Automation',
        ...
        glow: 'rgba(34, 197, 94, 0.12)' // Green
      }
    ];

    const contentPanel = document.getElementById('servicesContentPanel');
    const catEl = document.getElementById('serviceCat');
    const titleEl = document.getElementById('serviceTitle');
    const descEl = document.getElementById('serviceDesc');
    const bulletsEl = document.getElementById('serviceBullets');
    const linkEl = document.getElementById('serviceLink');
    const glowEl = document.getElementById('servicesGlow');

    let updateTimer = null;
    let activeIndexTracker = -1;

    const updateLeftPanel = (realIndex) => {
      if (typeof realIndex !== 'number' || realIndex < 0) return;
      const data = servicesData[realIndex];
      if (!data || !contentPanel || activeIndexTracker === realIndex) return;

      activeIndexTracker = realIndex;
      if (updateTimer) clearTimeout(updateTimer);

      contentPanel.classList.add('switching');
      ...
      updateTimer = setTimeout(() => {
        ...
        if (bulletsEl) {
          bulletsEl.innerHTML = data.bullets
            .map(b => `<li><i class="fa-solid fa-check"></i> <span>${b}</span></li>`)
            .join('');
        }

        if (glowEl) {
          glowEl.style.background = `radial-gradient(circle, ${data.glow} 0%, rgba(248, 250, 252, 0) 70%)`;
        }

        contentPanel.classList.remove('switching');
      }, 140);
    };
    ...
    const servicesSwiper = new Swiper('.services-swiper', {
      ...
      on: {
        init: function () {
          updateLeftPanel(this.realIndex);
        },
