(function() {
  ...
  const L_card = 2.7;        // length from top joint (p4) to card center (p5) (aligns ribbon end with hook top ring)
  ...
  const ribbonTexture = textureLoader.load('images/bandd.png?v=1.0.1');
  ...
  function createRibbonGeometry(curve, width) {
    ...
    const repeatX = -4.0; // matching repeat={[-4, 1]}
  ...
  const cardTexture = textureLoader.load('images/card_texture.png?v=' + Date.now(), (texture) => {
    texture.anisotropy = 16;
    texture.colorSpace = THREE.SRGBColorSpace;
    texture.flipY = true; // Enabled standard vertical mapping
  });

  gltfLoader.load('images/kartu.glb', (gltf) => {
    ...
    model.position.set(0.391783, 0.070779, -0.982853);
