document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleAISoundTrigger = (e) => {
    ...
    if (btn) {
      ...
      const id = btn.id || '';
      // Remove sound when clicking Start Chat button per user request
      if (btn.matches('#startChatSubmitBtn, #startChatBtn') || id.includes('start') || text.includes('start chat')) {
        return;
      }
      if (text.includes('book') || text.includes('consult')) {
        playUIBookSound();
      } else if (id.includes('submit') || id.includes('send') || id.includes('verify') || text.includes('send')) {
        playUIScfiSound();
      } else {
        playUIClickSound();
      }
  ...
  document.body.addEventListener('pointerdown', (e) => {
    if (e.button !== undefined && e.button !== 0) return; // Only left click or single touch
    handleAISoundTrigger(e);
  });
...
Every visitor should leave with a clear understanding of available services, confidence in Naveed's expertise, a personalized recommendation, a booked free consultation, and a positive impression of the business.`
      ...
      if (adminPin.value === '2421') {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (window.adjustForKeyboard) window.adjustForKeyboard();
          }, 50);
        }, 500);
      } else {
        alert("Incorrect PIN");
  ...
  if (preChatForm) {
    preChatForm.addEventListener('submit', (e) => {
      ...
      const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwzVDBoEFTXqE7ZHUuzI7IJTfa33MzpNeaLkEAarpiwB5GIVA-R56wXirnWdM4Qdhlskw/exec";
      const formData = {
          name: userData.name,
        ...
        .finally(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              playUIScfiSound(); // Play sci-fi sound right when the connection animation starts!
  ...
  const NVIDIA_API_KEY = "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";
  ...
  async function fetchExternalAI() {
    ...
    try {
      ...
      const proxyResponse = await fetch(GOOGLE_SCRIPT_URL, {
        method: 'POST',
        headers: {
      ...
      if (data && data.choices && data.choices.length > 0) {
        ...
        console.error("Proxy returned 403 Forbidden. Google Apps Script needs updated API key.", data);
      }
    } catch (proxyErr) {
      console.error("Proxy AI Error:", proxyErr);
    }
